...
(function($) {
    ...
    if (! window.HTM-ITUI.utils) {
        window.HTM-ITUI.utils = {};
    }

    /**
     * sync logScale function with mobile app
     */
    HTM-ITUI.utils.logScale = function(value) {
        if(value > 0.99999) {
            return 1;
        }
        return (
     ...
     */
    HTM-ITUI.utils.encodeXmlEntities = function(str) {
     ...
     */
    HTM-ITUI.utils.getUrlParam = function(key) {
     ...
     */
    HTM-ITUI.utils.isAuthorized = function() {
        return HTM-ITUI.utils.store.get('apiKey') ? true : false;
    };
     ...
     */
    HTM-ITUI.utils.isSetupFlow = function() {
        return HTM-ITUI.utils.getUrlParam('setup') ? true : false;
    };
     ...
     */
    HTM-ITUI.utils.isExpert = function() {
        return HTM-ITUI.utils.getUrlParam('expert') ? true : false;
    };

    /**
     * How many steps in basic setup vs. expert setup?
     */
    HTM-ITUI.utils.getSetupTotalSteps = function() {
        var type = HTM-ITUI.utils.isExpert() ? "expert" : "normal";
        return HTM-ITUI.msgs('site').setup.steps[type];
    };

    /**
     * Get Backbone.View for setup progress bar
     */
    HTM-ITUI.utils.getSetupProgressBar = function(step, el) {
        var total = HTM-ITUI.utils.getSetupTotalSteps(),
            percent = Math.floor(100 * (step - 1) / (total - 1));

        return new HTM-ITUI.SetupProgressBarView({
            el:      el,
            current: step,
            total:   total,
            percent: percent
     ...
     */
    HTM-ITUI.utils.modalError = function(error) {
        var regex = {
        ...
        if(message.match(regex.apiKey)) {
            HTM-ITUI.utils.store.clear('apiKey');
            HTM-ITUI.utils.go('/htm-it/welcome');
            return;
        }
    ...
    HTM-ITUI.utils.modalWarning = function(title, message, callback) {
        HTM-ITUI.utils.throb.stop();
    ...
    HTM-ITUI.utils.linkTo = function(page) {
        var sep = page.match(/^\//) ? '' : '/';
     ...
     * Go to another page
     */
    HTM-ITUI.utils.go = function(where, hardReset) {
        hardReset = true;   // TODO: remove when backbone views work together better
        if (hardReset) {
            window.location.href = HTM-ITUI.utils.linkTo(where);
        } else {
            HTM-ITUI.router.navigate(where, true);
        }
    };
     ...
     */
    HTM-ITUI.utils.title = function(msg) {
    ...
    HTM-ITUI.utils.help = {
        show : function() {
            ...
            var version = HTM-ITUI.product.version.replace(/(\d+)\.(\d+)\.?(\d*)/,"\$1.\$2");
            // Convert URL to version specific URL
            var url = HTM-ITUI.msgs('site').resources.help.replace(/%version%/g, version);
            window.open(url);
        }
    };
     ...
     *  across slots evenly.
     */
    HTM-ITUI.utils.makeGradientMap = function(slots, colors) {
        var colorMap =          [],
     ...
     */
    HTM-ITUI.utils.getDateParts = function(timestamp) {
     ...
     */
    HTM-ITUI.utils.getUTCDate = function(dateParts) {
     ...
     * @returns {object} JS UTC Date object instance
     */
    HTM-ITUI.utils.getUTCDateFromTimestamp = function(timestamp) {
        return HTM-ITUI.utils.getUTCDate(
            HTM-ITUI.utils.getDateParts(timestamp)
        );
    };
     ...
     */
    HTM-ITUI.utils.getUTCTimestamp = function(utcDateObj) {
