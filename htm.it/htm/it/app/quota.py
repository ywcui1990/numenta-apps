...
based on the running HTM-IT Edition (always 'Standard' in 1.5 and beyond)
...
class Quota(object):
  ...
  def init(cls):
    ...
    should be called once *before* starting HTM-IT services.
    """
    ...
    if editionType == "free":
      print ("As of HTM-IT 1.5, the 'free' edition is no longer supported; "
  ...
  def getInstanceQuota(cls):
    """
    retval: max number of active instances that may be monitored by this HTM-IT
    ...
    # We assume that Quota.init() is executed before HTM-IT services are started
    assert cls._cachedInstanceQuota > 0, (
      "Non-positive actual instance_quota=%s. Quota.init() not executed before "
      "HTM-IT services?") % (cls._cachedInstanceQuota,)
  ...
  def _getHostInfo(cls):
    if sys.platform.startswith("darwin"):
      ...
      # NOTE: assumes AWS/EC2 HTM-IT host instance
