    ...
    for _retries in xrange(20):
      try:
        with contextlib.closing(socket.socket()) as sock:
          sock.connect((htm-ithost, _PLAINTEXT_PORT))
...
def main(htm-ithost, apikey, minmodels, metricIncrement, csvFilePath,
  ...
  """
  gLog.info("Starting up: htm-ithost=%s; apikey=%s; minmodels=%d; "
            "csvFilePath=%s; feedforever=%s",
            htm-ithost, apikey, minmodels, csvFilePath, feedforever)
...
def _parseArgs():
  ...
  :returns: a dict;
    {"htm-ithost": <hostname or address>, "apikey": <api-key>,
  ...
  htm-ithost, apikey = posArgs

  return dict(htm-ithost=htm-ithost, apikey=apikey,
