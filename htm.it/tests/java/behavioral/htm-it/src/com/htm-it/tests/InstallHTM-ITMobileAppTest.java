...
public class InstallHTM-ITMobileAppTest {
    ...
    public static void installHTM-ITMobileAppTitleVerification(WebDriver driver) {
        String installHTM-ITMobileAppPageHelpText = TestUtilities.waitGetText(
                SETUP_COMPLETE_HELP_TEXT, driver, WAIT_TIME);
        String installHTM-ITMobileAppPageTitle = TestUtilities.waitGetText(
              SETUP_COMPLETE_TITLE, driver, WAIT_TIME);
        boolean checkContains1 = installHTM-ITMobileAppPageTitle.contains("Install HTM-IT Mobile App");
        Assert.assertEquals(checkContains1,true,"PAGE TITLE IS WRONG");
        Assert.assertEquals(installHTM-ITMobileAppPageHelpText,
                "Your server set up is now complete. "
        + "You need to install the Mobile App to begin using HTM-IT.");
    }

    public static void installHTM-ITMobileAppRequirementSection(WebDriver driver) {
        String installHTM-ITMobileAppPageRequirementTitle = TestUtilities
                .waitGetText(SETUP_COMPLETE_REQUIREMENT, driver, WAIT_TIME);
        String installHTM-ITMobileAppPageCompleteRequirementHelpText =
        TestUtilities.waitGetText(SETUP_COMPLETE_REQUIREMENT_HELP_TEXT_1,
            driver, WAIT_TIME);
        Assert.assertEquals(installHTM-ITMobileAppPageRequirementTitle,
                "Requirements:");
        Assert.assertEquals(
                installHTM-ITMobileAppPageCompleteRequirementHelpText,
                "Ensure that you are running Android 4.0.3 or later.");
    }

    public static void installHTM-ITMobileAppStepsVerification(WebDriver driver)
            throws InterruptedException {
        /*
         * 1st step under "Requirements" section i.e.
         * "Install the HTM-IT app via the Play Store by clicking"
         */
        String installHTM-ITMobileAppsteps1 = TestUtilities.waitGetText(
                SETUP_COMPLETE_STEP_1, driver, WAIT_TIME);
        String installHTM-ITMobileAppServerUrlLabel = TestUtilities.waitGetText(
                SETUP_COMPLETE_SERVER_URL, driver, WAIT_TIME);
        String installHTM-ITMobileAppPasswordLabel = TestUtilities.waitGetText(
                SETUP_COMPLETE_PASSWORD, driver, WAIT_TIME);
        /* 2nd step under "Requirements" section i.e. Tap the Sign In button. */
        String installHTM-ITMobileAppsteps2 = TestUtilities.waitGetText(
                SETUP_COMPLETE_STEP_2, driver, WAIT_TIME);
        /* 3rd step under"Requirements" section i.e. Finished! */
        String installHTM-ITMobileAppsteps3 = TestUtilities.waitGetText(
        ...
        Assert.assertEquals(installHTM-ITMobileAppsteps1,
                "Install the HTM-IT app via the Play Store by clicking:");
        Assert.assertEquals(installHTM-ITMobileAppServerUrlLabel, "Server URL");
        Assert.assertEquals(installHTM-ITMobileAppPasswordLabel,
                "Password (case sensitive)");
        Assert.assertEquals(installHTM-ITMobileAppsteps2,
                "Tap the Sign In button.");
        Assert.assertEquals(installHTM-ITMobileAppsteps3, "Finished!");
    }
     ...
     */
    public static void installHTM-ITMobileAppCompareRequirementSectionAndSetupServerURL(
            WebDriver driver) {
    ...
    public static void installHTM-ITMobileAppSetUpProgressBarVerification(WebDriver driver)
            throws InterruptedException {
        ReusableTests.testSetUpProgressText(driver);
    }

    public static void installHTM-ITAppFooter(WebDriver driver) throws Exception {
        ReusableTests.testFooter(driver);
    }

    public static void installHTM-ITAppHeader(WebDriver driver)
            throws InterruptedException {
        ReusableTests.testHeaderDuringSetup(driver);
    }

    public static void installHTM-ITMobileAppManageMonitoredInstancesButton(
