...
(function($) {
    ...
    if (! window.HTMITUI.utils) {
        window.HTMITUI.utils = {};
    }

    /**
     * sync logScale function with mobile app
     */
    HTMITUI.utils.logScale = function(value) {
        if(value > 0.99999) {
            return 1;
        }
        return (
     ...
     */
    HTMITUI.utils.encodeXmlEntities = function(str) {
     ...
     */
    HTMITUI.utils.getUrlParam = function(key) {
     ...
     */
    HTMITUI.utils.isAuthorized = function() {
        return HTMITUI.utils.store.get('apiKey') ? true : false;
    };
     ...
     */
    HTMITUI.utils.isSetupFlow = function() {
        return HTMITUI.utils.getUrlParam('setup') ? true : false;
    };
     ...
     */
    HTMITUI.utils.isExpert = function() {
        return HTMITUI.utils.getUrlParam('expert') ? true : false;
    };

    /**
     * How many steps in basic setup vs. expert setup?
     */
    HTMITUI.utils.getSetupTotalSteps = function() {
        var type = HTMITUI.utils.isExpert() ? "expert" : "normal";
        return HTMITUI.msgs('site').setup.steps[type];
    };

    /**
     * Get Backbone.View for setup progress bar
     */
    HTMITUI.utils.getSetupProgressBar = function(step, el) {
        var total = HTMITUI.utils.getSetupTotalSteps(),
            percent = Math.floor(100 * (step - 1) / (total - 1));

        return new HTMITUI.SetupProgressBarView({
            el:      el,
            current: step,
            total:   total,
            percent: percent
     ...
     */
    HTMITUI.utils.modalError = function(error) {
        var regex = {
    ...
    HTMITUI.utils.modalWarning = function(title, message, callback) {
        HTMITUI.utils.throb.stop();
    ...
    HTMITUI.utils.linkTo = function(page) {
        var sep = page.match(/^\//) ? '' : '/';
     ...
     * Go to another page
     */
    HTMITUI.utils.go = function(where, hardReset) {
        hardReset = true;   // TODO: remove when backbone views work together better
        if (hardReset) {
            window.location.href = HTMITUI.utils.linkTo(where);
        } else {
            HTMITUI.router.navigate(where, true);
        }
    };
     ...
     */
    HTMITUI.utils.title = function(msg) {
    ...
    HTMITUI.utils.help = {
        show : function() {
            ...
            var version = HTMITUI.product.version.replace(/(\d+)\.(\d+)\.?(\d*)/,"\$1.\$2");
            // Convert URL to version specific URL
            var url = HTMITUI.msgs('site').resources.help.replace(/%version%/g, version);
            window.open(url);
        }
    };
     ...
     *  across slots evenly.
     */
    HTMITUI.utils.makeGradientMap = function(slots, colors) {
        var colorMap =          [],
     ...
     */
    HTMITUI.utils.getDateParts = function(timestamp) {
     ...
     */
    HTMITUI.utils.getUTCDate = function(dateParts) {
     ...
     * @returns {object} JS UTC Date object instance
     */
    HTMITUI.utils.getUTCDateFromTimestamp = function(timestamp) {
        return HTMITUI.utils.getUTCDate(
            HTMITUI.utils.getDateParts(timestamp)
        );
    };
     ...
     */
    HTMITUI.utils.getUTCTimestamp = function(utcDateObj) {
