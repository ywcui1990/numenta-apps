...
(function() {
    ...
    /*************************************************************************
     * IMPORTANT!
     * Hyde (http://hyde.github.com) has a problem processing the regular
    ...
    /**
     * Helper function for GROKUI.msgs. Used to inject an object full of data
     ...
     * We're defining a function call GROKUI.msgs, which will also act as a
     * key-value store for messages. Users will use it like this:
     *  GROKUI.msgs('msg-key') // to return a raw msgs object for a template
     * Or:
     *  GROKUI.msgs('msg-key', {user: {firstName: 'Steve'}}
     *
     ...
     * the token { {user.firstName} }, GROKUI.msgs will replace that value with
     ...
     * @return {*}
     */
    GROKUI.msgs = function(key, subs) {
        if (! subs) {
            return GROKUI.msgs[key];
        }
        return constructMessage(GROKUI.msgs[key], subs);
    };
            ...
            $.ajax({
                ...
                success: function(msgs) {
                    ...
                    Object.keys(msgs).forEach(function(msgKey) {
                        ...
                        GROKUI.msgs[msgKey] = msgs[msgKey];
