...
from grok.app import product, repository
...
class ModelHandler(AuthenticatedBaseHandler):
  ...
  def upgradeCustomModelSpec(cls, modelSpec):
    """ Upgrade legacy grok custom metric modelSpec to the current format

    :param modelSpec: legacy grok custom metric modelSpec
    ::
        {
          "datasource": "custom",
    ...
    :returns: grok custom metric modelSpec in the current format (see
