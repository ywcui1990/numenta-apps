...
based on the running Grok Edition (always 'Standard' in 1.5 and beyond)
...
from grok.app import GROK_HOME, product, repository
...
class Quota(object):
  ...
  def getInstanceQuota(cls):
    """
    retval: max number of active instances that may be monitored by this Grok
    ...
    # We assume that Quota.init() is executed before Grok services are started
    assert cls._cachedInstanceQuota > 0, (
      "Non-positive actual instance_quota=%s. Quota.init() not executed before "
      "Grok services?") % (cls._cachedInstanceQuota,)
  ...
  def _getHostInfo(cls):
    if sys.platform.startswith("darwin"):
      ...
      # NOTE: assumes AWS/EC2 Grok host instance
