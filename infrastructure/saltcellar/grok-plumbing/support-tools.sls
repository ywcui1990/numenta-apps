...
# ----------------------------------------------------------------------
# Formula: grok-plumbing.support-tools

# Install everything needed for remote support of Grok

# These directories are required by gs-support-access-tool
# and gs-get-support-keys
{% for dirpath in ['/etc/grok/sshkeys.d',
                   '/etc/grok/sshkeys.d/originals'] %}
{{ dirpath }}:
  file.directory:
    - user: root
    - group: root
...
gs-support-access-tool:
  file.managed:
    ...
    - source: salt://grok-plumbing/files/support-tools/gs-support-access-tool
    - user: root
    - group: root
    - mode: 0755

# Install helper script to download the Grok support public keys from S3
gs-get-support-keys:
  file.managed:
    - name: /usr/local/sbin/gs-get-support-keys
    - source: salt://grok-plumbing/files/support-tools/gs-get-support-keys
    - user: root
    - group: root
    - mode: 0755
