...
# ----------------------------------------------------------------------
"""
Test Grok AMI. Ensure OpenSSL hack is installed, and that our private
...
class TestGrokAMI(unittest.TestCase):

  def testMySQLCommunityRepoMustBePresent(self):
    """Confirm that the mysql community repo files are on the Grok AMI."""
    # Check for the GPG key
    self.assertTrue(os.path.exists("/etc/pki/rpm-gpg/RPM-GPG-KEY-mysql"),
                     "mysql community repo must be on Grok marketplace AMIs")
  ...
  def testSecretsauceRepoMustBeAbsent(self):
    """Confirm that the secretsauce repo file is not on the Grok AMI."""
