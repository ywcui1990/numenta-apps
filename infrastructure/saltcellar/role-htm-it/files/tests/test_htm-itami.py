...
# ----------------------------------------------------------------------
"""
Test HTM-IT AMI. Ensure OpenSSL hack is installed, and that our private
...
class TestHTMITAMI(unittest.TestCase):
  ...
  def testMySQLCommunityRepoMustBePresent(self):
    ...
    self.assertTrue(os.path.exists("/etc/pki/rpm-gpg/RPM-GPG-KEY-mysql"),
                     "mysql community repo must be on HTM-IT marketplace AMIs")
  ...
  def testSecretsauceRepoMustBeAbsent(self):
    """Confirm that the secretsauce repo file is not on the HTM-IT AMI."""
