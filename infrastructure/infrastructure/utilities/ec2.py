...
def launchInstance(amiID, config, logger):
  ...
  # `htm-it_server` allows access to the server from ports 80 & 443 universally,
  # but can be limited if necessary.
  if config["REGION"] == "us-west-2":
    securityGroups = ["basic_server", "htm-it_server"]
  if config["REGION"] == "us-east-1":
    securityGroups = ["basic_server", "htm-it_server_east"]
