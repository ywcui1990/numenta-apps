...
import com.numenta.htmit.mobile.service.MetricParser;
...
public class MetricParserTests extends CoreAndroidTestCase {
    ...
    public void testParseSingleMetric_1_3() {
        ...
        try {
            InputStream in = getTestData(HTMITClientImpl.SERVER_1_3, "single_model.json");
    ...
    public void testParseSingleCustomMetric_1_6() {
        JsonReader reader = null;
        try {
            InputStream in = getTestData(HTMITClientImpl.SERVER_1_6, "single_custom_model.json");
            reader = new JsonReader(new InputStreamReader(in, "UTF-8"));
            MetricParser parser = new MetricParser(reader);
            List<Metric> metrics = parser.parse();
            assertEquals(metrics.size(), 1);
    ...
    public void testParseMultiMetric_1_3() {
        JsonReader reader = null;
        try {
            InputStream in = getTestData(HTMITClientImpl.SERVER_1_3, "multi_model.json");
            reader = new JsonReader(new InputStreamReader(in, "UTF-8"));
            MetricParser parser = new MetricParser(reader);
            List<Metric> metrics = parser.parse();
    ...
    public void testParseInvalidMetric_1_3() {
        JsonReader reader = null;
        try {
            InputStream in = getTestData(HTMITClientImpl.SERVER_1_3, "invalid_model.json");
            reader = new JsonReader(new InputStreamReader(in, "UTF-8"));
            MetricParser parser = new MetricParser(reader);
            parser.parse();
    ...
    public void testParseUserInfo_1_6() {
        JsonReader reader = null;
        try {
            InputStream in = getTestData(HTMITClientImpl.SERVER_1_6, "metric_user_info.json");
            reader = new JsonReader(new InputStreamReader(in, "UTF-8"));
            MetricParser parser = new MetricParser(reader);
            List<Metric> metrics = parser.parse();
            assertEquals(metrics.size(), 1);
