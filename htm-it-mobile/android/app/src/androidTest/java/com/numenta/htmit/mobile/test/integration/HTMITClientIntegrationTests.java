...
package com.numenta.htmit.mobile.test.integration;

import com.numenta.htmit.mobile.BuildConfig;
import com.numenta.htmit.mobile.HTMITApplication;
import com.numenta.htmit.mobile.service.HTMITClientImpl;
import com.numenta.htmit.mobile.service.InstanceParser;
...
// FIXME: TAUR-601 Move all HTM-IT integration tests to the HTM-IT folder path
public class HTMITClientIntegrationTests extends ApplicationTestCase<HTMITApplication> {

    // Server Info
    HTMITClientImpl _client;
    ...
    public HTMITClientIntegrationTests() {
    ...
    private void beforeClass() {
        ...
        try {
            _client = (HTMITClientImpl) HTMITApplication.getInstance().connectToServer(_serverUrl, _serverPass);
    ...
    public final void testAnnotationBackwardCompatibility() throws IOException, HTMException {
        ...
        if (_client.getServerVersion().compareTo(HTMITClientImpl.SERVER_1_6) >= 0) {
    ...
    public final void testGetServerVersion() {
        assertTrue("Server must be " + HTMITClientImpl.SERVER_OLDEST + " or greater",
                _client.getServerVersion().compareTo(HTMITClientImpl.SERVER_OLDEST) >= 0);
    }
