...
package com.numenta.htmit.mobile.service;
...
import static com.numenta.htmit.mobile.preference.PreferencesConstants.PREF_EMAIL_NOTIFICATIONS_ENABLE;
import static com.numenta.htmit.mobile.preference.PreferencesConstants.PREF_NOTIFICATIONS_EMAIL;
import static com.numenta.htmit.mobile.preference.PreferencesConstants.PREF_NOTIFICATIONS_FREQUENCY;
import static com.numenta.htmit.mobile.preference.PreferencesConstants.PREF_NOTIFICATION_INITIALIZED;
import static com.numenta.htmit.mobile.preference.PreferencesConstants.PREF_NOTIFICATION_NEED_UPDATE;
import static com.numenta.htmit.mobile.preference.PreferencesConstants.PREF_PASSWORD;
import static com.numenta.htmit.mobile.preference.PreferencesConstants.PREF_SERVER_URL;
 ...
 */
public class HTMITNotificationService extends com.numenta.core.service.NotificationService {

    private static final String TAG = HTMITNotificationService.class.getSimpleName();
    ...
    public HTMITNotificationService(DataService service) {
        super(service);
    }
        ...
        if (groupNotifications) {
            if (enable) {
                for (Notification notification : pendingNotifications) {
                    ...
                    if (description == null) {
                        metric = database.getMetric(notification.getMetricId());
                        if (metric != null) {
                            value = database.getMetricValue(metric.getId(),
                    ...
                    localId = database.addNotification(notification.getNotificationId(),
            ...
            for (Notification notification : pendingNotifications) {
                ...
                metric = database.getMetric(notification.getMetricId());
                if (metric != null) {
                    value = database.getMetricValue(metric.getId(), notification.getTimestamp());
                    ...
                    if (localId != -1) {
                        ...
                        if (enable) {
                            notificationCount = database.getUnreadNotificationCount();
     ...
     */
    @Override
    public HTMITClientImpl getClient() throws IOException, HTMException {
        return (HTMITClientImpl)super.getClient();
    }
