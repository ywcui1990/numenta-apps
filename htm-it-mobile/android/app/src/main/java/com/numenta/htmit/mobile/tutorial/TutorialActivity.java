...
package com.numenta.htmit.mobile.tutorial;
...
public class TutorialActivity extends FragmentActivity {
    ...
    private final BroadcastReceiver _metricChangedReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            // Cache last metric change event while tutorial is active
            _instances = intent.getIntExtra(HTMITDataSyncService.EXTRA_NEW_INSTANCES, 0);
            _metrics = intent.getIntExtra(HTMITDataSyncService.EXTRA_NEW_METRICS, 0);
            _remaining = intent.getIntExtra(HTMITDataSyncService.EXTRA_REMAINING_TIME, 0);
        }
    };
        ...
        if (_metrics > 0) {
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    intent.putExtra(HTMITDataSyncService.EXTRA_NEW_METRICS, _metrics);
                    intent.putExtra(HTMITDataSyncService.EXTRA_NEW_INSTANCES, _instances);
                    intent.putExtra(HTMITDataSyncService.EXTRA_REMAINING_TIME, _remaining);
