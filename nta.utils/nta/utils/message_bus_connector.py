...
class MessageProperties(amqp.BasicProperties):
...
class MessageBusConnector(object):
  ...
  _PERSISTENT_PUBLISH_PROPERTIES = amqp.BasicProperties(
  ...
  def __init__(self):
    ...
    def configureChannel(client):
      ...
      :param SynchronousAmqpClient client:
      """
  ...
  def getAllMessageQueues(self):
    ...
    """
    connectionParams = amqp.RabbitmqManagementConnectionParams()
  ...
  def publishExg(self,
    ...
    # NOTE: when using the default exchange (""), the the routing key is used
    #   to select the destination queue
    msg = amqp.Message(body, properties=properties)
    try:
...
class _QueueConsumer(object):
  ...
  def __iter__(self):
    ...
    def consumeBlocking(amqpClient, mqName):
      ...
      try:
        while True:
          ...
          if type(evt) is amqp.ConsumerMessage:
          ...
          elif type(evt) is amqp.ConsumerCancellation:
    ...
    while not done:
      ...
      except (amqp.AmqpChannelError, amqp.AmqpConnectionError) as e:
        if isinstance(e, amqp.AmqpChannelError):
          if e.code == AMQPErrorCodes.NOT_FOUND:
...
class _ChannelManager(object):
  ...
  def _connectToBrokerNoRetries(self):
    connectionParams = amqp.getRabbitmqConnectionParameters()
