...
* Current workaround to sign the electron app: if you have a "Developer ID Application" certificate you can add `--sign=<your certificate common name>` at the end the `electron-packager` command of the `build` script in `package.json`. This will build and sign the app.
