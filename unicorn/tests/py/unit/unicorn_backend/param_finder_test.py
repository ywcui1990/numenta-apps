...
class ParamFinderTestCase(unittest.TestCase):
  ...
  def testFindParameters(self):
    def createTestData(type, timeStepSeconds=300):
      dayPeriod = 86400.0
      weekPeriod = 604800.0
      ...
      for i in xrange(1000):
        timeStamps.append(startTime + timeStep * i)
        if type == "flat":
          values.append(10.0)
        if type == "daily":
          values.append(numpy.sin(2 * numpy.pi *
                                  (timeStep.seconds * i) / dayPeriod))
        if type == "weekly":
          values.append(numpy.sin(2 * numpy.pi *
                                  (timeStep.seconds * i) / weekPeriod))
      return (timeStamps, values)


    outputInfo = param_finder.find_parameters(createTestData('flat', 300))
    self.assertTrue(outputInfo["aggInfo"]['windowSize'] > 300)
    self.assertTrue(outputInfo["aggInfo"]['func'] == 'mean')
    ...
    outputInfo = param_finder.find_parameters(createTestData('daily', 300))
    self.assertTrue(outputInfo["aggInfo"]['windowSize'] > 300)
    self.assertTrue(outputInfo["aggInfo"]['func'] == 'mean')
    # Use timeOfDay but not dayOfWeek encoder for dataSet with daily period
    self.assertFalse(outputInfo["modelInfo"]['modelConfig']['modelParams'] \
                       ['sensorParams']['encoders']['c0_timeOfDay'] == None)
    self.assertTrue(outputInfo["modelInfo"]['modelConfig']['modelParams'] \
                      ['sensorParams']['encoders']['c0_dayOfWeek'] == None)

    outputInfo = param_finder.find_parameters(createTestData('weekly', 7200))
    self.assertTrue(outputInfo["aggInfo"]['windowSize'] > 7200)
    self.assertTrue(outputInfo["aggInfo"]['func'] == 'mean')
